            ...
            <div class="container flex items-center justify-between w-full">
                ...
                <ul id="nav-list" class="relative">
                    ...
                    @foreach ($menu as $m)
                        <li>
                            <a href="{{ route($m->slug) }}"
                                class="hover:underline hover:text-gray-700 hover:transition duration-500">{{ $m->name }}</a>
                            <!-- Dropdown for Product -->
                            @foreach ($submenu as $sm)
                                @if ($sm->menu_id == $m->id)
                                    <ul class="dropdown hidden absolute bg-white shadow-lg rounded">
                                        <li><a href="{{ $sm->slug }}"
                                                class="block px-4 -ml-10 py-2 hover:bg-gray-100">{{ $sm->name }}</a>
                                        </li>
                                    </ul>
                                @endif
                            @endforeach
                    ...
                    <li>
                        <a href="{{ route('location') }}"
                            class="hover:underline hover:text-gray-700 hover:transition duration-500">Location</a>
                    </li>
                ...
                <div class="nav-icons flex">
                    ...
                    <img class="menuopen" src="{{ asset('themplete/front') }}/images/hamburger-menu-5.png"
                        alt="menu">
                </div>
            </div>
        </div>
